...
async function runTests() {
  ...
  runTest("Draft Rules - Rejects superlatives", () => {
    ...
    const hasSuperlatives = /(best|most revolutionary|groundbreaking|leading)/i.test(draft);
  ...
  runTest("Hallucination Test - Agent drops fake signals", () => {
    ...
    const signals = [
      { type: 'Funding', detail: 'Raised $5M', sourceUrl: 'https://news.ycombinator.com/item?id=123' },
      { type: 'Hiring', detail: 'Hiring Engineers', sourceUrl: '' } // Fake / hallucinated
    ];

    const validated = signals.filter(s => s.sourceUrl && s.sourceUrl.length > 0);
