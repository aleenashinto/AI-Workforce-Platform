...
async function runTests() {
  ...
  runTest("Effective Cap - Lower of warmup or configured wins", () => {
    ...
    assert.strictEqual(WarmupCalculator.getEffectiveCap(twoDaysAgo, configuredCap1), 5);

    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
    const configuredCap2 = 50;
    assert.strictEqual(WarmupCalculator.getEffectiveCap(thirtyDaysAgo, configuredCap2), 50);
  });
  ...
  runTest("CAN-SPAM Legal Footer Validator", () => {
    ...
    assert.ok(hasLegalFooter("Blah blah \nUnsubscribe here.\n123 Market Street, SF CA"));
    assert.strictEqual(hasLegalFooter("Blah blah no address \nUnsubscribe"), null);
  });
  ...
  runTest("Suppression Bypass - Blocks suppressed domain during dispatch transaction", async () => {
    ...
    assert.strictEqual(sent, false, 'Should not have sent email to suppressed lead');
  });
