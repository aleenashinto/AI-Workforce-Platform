  ...
  try {
    ...
    const seqs = [
      {
        ...
        name: "Enterprise SaaS Outbound — Q3",
        status: "active",
        goal: "Generate qualified sales meetings from enterprise SaaS leads.",
      },
      {
        id: crypto.randomUUID(),
        name: "Inbound Lead Nurture",
        status: "paused",
        goal: "Nurture leads who downloaded the whitepaper.",
      },
      {
        id: crypto.randomUUID(),
        name: "No Response Follow-up",
        status: "draft",
        goal: "Re-engage dead leads.",
      },
    ];
    ...
    for (const s of seqs) {
      await sqlClient`
        ...
        VALUES (${s.id}, ${org_id}, ${s.name}, 'Demo sequence', ${s.goal}, ${s.status}, 1)
      `;

      // Add steps
      await sqlClient`
        INSERT INTO sequence_steps (id, sequence_id, day_offset, name, type, channel, template)
        VALUES (${crypto.randomUUID()}, ${s.id}, 0, 'Initial Outreach', 'email', 'email', 'Subject: Hi\nBody: Hello')
      `;
      await sqlClient`
        INSERT INTO sequence_steps (id, sequence_id, day_offset, name, type, channel, template)
        VALUES (${crypto.randomUUID()}, ${s.id}, 2, 'Wait 2 Days', 'wait', 'wait', '')
      `;
      await sqlClient`
        INSERT INTO sequence_steps (id, sequence_id, day_offset, name, type, channel, template)
        VALUES (${crypto.randomUUID()}, ${s.id}, 2, 'AI Follow-up', 'ai_email', 'email', 'Subject: Follow up\nBody: AI Generated')
      `;
      ...
      if (s.status !== "draft") {
        ...
        for (let i = 0; i < leads.length; i++) {
          ...
          await sqlClient`
            ...
            VALUES (${crypto.randomUUID()}, ${org_id}, ${s.id}, ${leads[i].id}, ${status}, ${currentStep})
          `;
