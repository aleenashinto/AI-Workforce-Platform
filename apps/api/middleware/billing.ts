...
export const checkBillingQuota = async (
  ...
  try {
    ...
    const [subscription] =
      await sql`SELECT stripe_subscription_id, plan_id, status FROM organizations WHERE id = ${org_id}`;
    ...
    if (
      ...
      reply
        ...
        .send({
          error:
            "Payment Required: Your subscription is past due or canceled. Please update your billing.",
        });
    ...
    if (usage && parseInt(usage.count) >= quota) {
      reply
        ...
        .send({
          error:
            "Quota Exceeded: You have reached your monthly API usage limit.",
        });
      return reply;
    }
    ...
    sql`INSERT INTO usage_events (id, org_id, event_type, created_at) VALUES (gen_random_uuid(), ${org_id}, 'api_call', NOW())`.catch(
