...
export default async function authRoutes(fastify: FastifyInstance) {
  ...
  if (isMockOAuth) {
    fastify.get("/auth/google/login", async (req, reply) => {
      return reply.redirect(
        `${process.env.API_URL || "http://localhost:3001"}/auth/google/callback?code=mock_code`,
      );
    });
  } else {
  ...
  const handleOAuthCallback = async (
    ...
    profileMapper: (data: any) => { id: string; email: string; name: string },
  ) => {
    try {
      let data = {};
      ...
      let [user] = await db
        .select()
        .from(users)
        .where(eq(users.auth_provider_id, authProviderId))
        .limit(1);
      ...
      if (!user) {
        ...
        if (!user) {
          ...
          await db
            .update(users)
            .set({ auth_provider_id: authProviderId })
      ...
      const frontendBaseUrl = (
        ...
        .replace(/\/+$/, "")
        .replace(/\/platform\/dashboard$/, "")
        .replace(/\/dashboard$/, "");
      return reply.redirect(`${frontendBaseUrl}/dashboard`);
    } catch (err) {
      fastify.log.error(err);
      const frontendBaseUrlError = (
        process.env.FRONTEND_URL || "http://localhost:3000"
      )
        .replace(/\/+$/, "")
        .replace(/\/platform\/dashboard$/, "")
  ...
  fastify.get("/auth/google/callback", async (req, reply) => {
    ...
    if (!isMockOAuth) {
      token = await (
        fastify as any
      ).googleOAuth2.getAccessTokenFromAuthorizationCodeFlow(req).token;
    }
  ...
  fastify.post(
    ...
    async (req: any, reply) => {
      ...
      try {
        ...
        fastify.log.error(
          { errCode: err.code, msg: err.message },
      ...
      try {
        ...
        fastify.log.error(
          { errCode: err.code, msg: err.message },
          "Database error during org/membership creation",
        );
      }
  ...
  fastify.post(
    ...
    async (req: any, reply) => {
      ...
      if (!membership) {
        return reply
          .status(400)
          .send({ error: "Account has no associated organization" });
      }
  ...
  fastify.post("/auth/verify-email", async (req: any, reply) => {
    ...
    await db
      ...
      .where(eq(users.id, verifyToken.user_id));
    await db
      .update(email_verification_tokens)
      .set({ used: true })
      .where(eq(email_verification_tokens.id, verifyToken.id));
  ...
  fastify.post("/auth/invite/accept", async (req: any, reply) => {
    ...
    try {
      ...
      return reply
        .status(401)
        .send({ error: "Must be logged in to accept invitation" });
    }
    ...
    const [existingMembership] = await db
      ...
      .where(
        and(
          ...
          eq(memberships.org_id, invite.org_id),
        ),
      )
      .limit(1);
