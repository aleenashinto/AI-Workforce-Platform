...
export default async function crmRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/settings', async (request, reply) => {
    const { org_id } = (request as any).user || { org_id: '00000000-0000-0000-0000-000000000001' };
    const { category } = request.query as any || { category: 'crm' };
    
    const settings = await db
      .select()
      .from(integrations)
      .where(and(eq(integrations.org_id, org_id), eq(integrations.category, category)));
  ...
  fastify.post('/settings', async (request, reply) => {
    ...
    const [existing] = await db
      .select()
      .from(integrations)
      .where(and(eq(integrations.org_id, org_id), eq(integrations.provider, provider)))
      .limit(1);

    if (existing) {
      const [updated] = await db.update(integrations)
        .set({ credentials, config, sync_status: 'active', updated_at: new Date() })
        .where(eq(integrations.id, existing.id))
        .returning();
      return { success: true, data: updated };
    } else {
      const [inserted] = await db.insert(integrations)
        .values({
          id: uuidv4(),
          org_id,
          category,
  ...
  fastify.post('/sync', async (request, reply) => {
    ...
    const [updated] = await db.update(integrations)
      .set({ last_sync_at: new Date(), sync_status: 'active', updated_at: new Date() })
      .where(and(eq(integrations.org_id, org_id), eq(integrations.provider, provider)))
      .returning();
