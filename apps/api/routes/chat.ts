...
export async function chatRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/v1/chat', async (request, reply) => {
    ...
    if (!query || !conversation_id) {
      return reply.status(400).send({ error: 'Query and conversation_id are required' });
    }
    ...
    const [conversation] = await sql`SELECT ai_paused FROM conversations WHERE id = ${conversation_id} AND org_id = ${org_id}`;
    if (conversation && conversation.ai_paused) {
      return reply.status(403).send({ error: 'Conversation is currently paused for human takeover' });
    }
