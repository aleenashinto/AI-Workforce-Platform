...
export default async function agentRoutes(fastify: FastifyInstance) {
  ...
  fastify.get("/conversations", async (req, reply) => {
    ...
    let orgConversations = await db
      ...
      .leftJoin(
        end_users,
        eq(conversations.visitor_id, sql<string>`${end_users.id}::text`),
      )
    ...
    if (search && search.trim() !== "") {
      ...
      orgConversations = orgConversations.filter((row) => {
        return (
          row.end_user?.name?.toLowerCase().includes(q) ||
    ...
    const unassigned = orgConversations
      .filter((r) => !r.conversation.assigned_to)
      .map((r) => ({ ...r.conversation, end_user: r.end_user }));
    const assigned = orgConversations
      .filter((r) => !!r.conversation.assigned_to)
      .map((r) => ({ ...r.conversation, end_user: r.end_user }));
  ...
  fastify.post("/conversations/:id/copilot", async (req, reply) => {
    ...
    try {
      if (lastUserMessage.trim()) {
        ...
        const topKnowledge = await db
          .select({
            ...
            distance:
              sql`(${knowledge_chunks.embedding} <=> ${vectorStr}::vector)`.mapWith(
                Number,
              ),
          })
          .from(knowledge_chunks)
          .orderBy(
            sql`(${knowledge_chunks.embedding} <=> ${vectorStr}::vector)`,
          )
          .limit(3);
      ...
      console.warn(
        "Knowledge retrieval failed (possibly no API key or empty vector DB), falling back to baseline.",
        err,
      );
    }
