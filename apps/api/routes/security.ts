...
const securityRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  fastify.get(
    "/keys",
    { preHandler: requireAction("MANAGE_API_KEYS") },
    async (request, reply) => {
      const org_id =
      ...
      const keys = await withTenant(db, org_id, async (tx: any) => {
        return tx
          ...
          .from(api_keys)
          .where(and(eq(api_keys.org_id, org_id), sql`revoked_at IS NULL`));
      });
      return keys;
    },
  );
  ...
  fastify.post(
    "/export",
    { preHandler: requireAction("MANAGE_SETTINGS") },
    async (request, reply) => {
      const { org_id, user_id } = request.user as any;
      ...
      return {
        success: true,
        message:
          "Data export initiated. You will receive an email when it is ready.",
      };
    },
  );
  ...
  fastify.patch(
    "/approval-gate",
    { preHandler: requireAction("TOGGLE_APPROVAL_GATE") },
    async (request, reply) => {
      const { org_id, user_id } = request.user as any;
