...
export async function analyticsRoutes(fastify: FastifyInstance) {
  fastify.get("/analytics/support-overview", async (request, reply) => {
    ...
    if (start_date)
      conditions.push(gte(conversations.created_at, new Date(start_date)));
    if (end_date)
      conditions.push(lte(conversations.created_at, new Date(end_date)));
    const whereClause = and(...conditions);

    try {
      // 1. KPIs
      const totalConvRes = await db
      ...
      const aiRes = await db
        ...
        .where(
          and(
            ...
            sql`${conversations.assigned_to} IS NULL`,
          ),
        );
      ...
      const escRes = await db
        .select({ count: sql<number>`cast(count(*) as integer)` })
        .from(conversations)
        .where(and(whereClause, eq(conversations.status, "escalated")));
      const escalated = escRes[0].count || 0;

      const csatRes = await db
        .select({
          avg: sql<number>`avg(cast(${conversations.csat_score} as float))`,
        })
        .from(conversations)
        .where(whereClause);
      ...
      const volumeRes = await db
        ...
        .from(conversations)
        .where(whereClause)
        .groupBy(sql`to_char(${conversations.created_at}, 'YYYY-MM-DD')`)
        .orderBy(sql`1`);
        ...
        recent.map(async (c) => {
          ...
          if (c.assigned_to) {
            const ag = await db
              ...
              .from(users)
              .where(eq(users.id, c.assigned_to))
              .limit(1);
  ...
  fastify.get("/analytics/knowledge-gaps", async (request, reply) => {
    ...
    try {
      ...
      return reply
        .status(500)
        .send({ error: "Failed to fetch knowledge gaps" });
    }
  });
}
