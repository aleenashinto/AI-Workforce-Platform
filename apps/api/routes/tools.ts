...
export default async function toolsRoutes(fastify: FastifyInstance) {
  fastify.get('/v1/tools', async (req, reply) => {
    const { org_id } = (req as any).user || { org_id: '00000000-0000-0000-0000-000000000001' };
  ...
  fastify.post('/v1/tools', async (req, reply) => {
    const { org_id } = (req as any).user || { org_id: '00000000-0000-0000-0000-000000000001' };
    const { name, description, endpoint, auth_header, schema, requires_confirmation, is_active } = req.body as any;
