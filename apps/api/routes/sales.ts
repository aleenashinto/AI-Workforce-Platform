...
import { leads, campaigns, outreach_events, icps } from '@ai-workforce/db/schema';
...
try {
  ...
  researchQueue = new Queue('research-queue', { 
    connection: { url: process.env.REDIS_URL || 'redis://localhost:6379', lazyConnect: true, retryStrategy: () => null, maxRetriesPerRequest: null } 
  });
} catch (e) {
...
export default async function salesRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/icps/generate', { preHandler: requireAction('MANAGE_CAMPAIGNS') }, async (request, reply) => {
    ...
    return {
      ...
      data: {
        ...
        persona: { titles: ['CTO', 'VP Engineering', 'Head of Data'], seniority: 'Director+' },
  ...
  fastify.get('/leads', async (request, reply) => {
    ...
    try {
      ...
      const allLeads = await db.select().from(leads).where(and(...conditions)).orderBy(desc(leads.created_at));
  ...
  fastify.get('/leads/:id', async (request, reply) => {
    ...
    try {
      const lead = await db.select().from(leads).where(and(eq(leads.id, id), eq(leads.org_id, org_id))).limit(1);
      if (!lead.length) return reply.code(404).send({ error: 'Lead not found' });
  ...
  fastify.post('/leads/discover', { preHandler: requireAction('MANAGE_LEADS') }, async (request, reply) => {
    ...
    try {
        ...
        for (const contact of contacts) {
          ...
          mockLeads.push({
            ...
            signals: [
              ...
              { id: '2', text: 'Hiring for Engineering', url: 'https://boards.greenhouse.io', date: new Date().toISOString(), confidence: 0.88 }
            ],
      ...
      return { success: true, message: `Discovered and verified ${mockLeads.length} leads`, data: mockLeads };
    } catch (error: any) {
      request.log.error(error);
  ...
  fastify.post('/leads/:id/research', { preHandler: requireAction('MANAGE_LEADS') }, async (request, reply) => {
    ...
    try {
      ...
      if (researchQueue) {
        await researchQueue.add('research-lead', { leadId: id, orgId: org_id });
      } else {
