...
export function Widget({ config }: { config: any }) {
              ...
              if (line.startsWith('data: ')) {
                try {
                  ...
                  if (data.metadata?.citations) {
                     setMessages(prev => prev.map(msg => 
                      msg.id === assistantMsgId ? { ...msg, citations: data.metadata.citations } : msg
                    ));
                  }
                } catch (e) {
  ...
  const renderContentWithCitations = (content: string) => {
    ...
    return parts.map((part, i) => {
      if (part.match(/^\[\d+\]$/)) {
        return <span key={i} className="aw-citation" title="View Source">{part}</span>;
      }
    ...
    >
      <div className={`aw-chat-window ${isOpen ? 'aw-open' : ''}`}>
        <div className="aw-header" style={{ background: config.color }}>
          ...
          <button className="aw-close-btn" onClick={() => setIsOpen(false)}>✕</button>
        </div>
        <div className="aw-messages">
          {messages.map(msg => (
            <div key={msg.id} className={`aw-message aw-${msg.role}`} style={msg.role === 'assistant' ? { background: config.color } : {}}>
              {renderContentWithCitations(msg.content)}
            </div>
          ))}
          {isLoading && (
            <div className="aw-message aw-assistant" style={{ background: config.color, opacity: 0.7 }}>
        ...
        <form className="aw-input-area" onSubmit={handleSubmit}>
          <input 
            ...
            disabled={isLoading}
          />
          <button type="submit" className="aw-send-btn" disabled={isLoading || !input.trim()}>
