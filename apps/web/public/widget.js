(function () {
  var e,
    t,
    n,
    r,
    i,
    a,
    o,
    s,
    c,
    l,
    u,
    d,
    f,
    p,
    m = {},
    h = [],
    g = /acit|ex(?:s|g|n|p|$)|rph|grid|ows|mnc|ntw|ine[ch]|zoo|^ord|itera/i,
    _ = Array.isArray;
  function v(e, t) {
    for (var n in t) e[n] = t[n];
    return e;
  }
  function y(e) {
    e && e.parentNode && e.parentNode.removeChild(e);
  }
  function b(t, n, r) {
    var i,
      a,
      o,
      s = {};
    for (o in n)
      o == `key` ? (i = n[o]) : o == `ref` ? (a = n[o]) : (s[o] = n[o]);
    if (
      (arguments.length > 2 &&
        (s.children = arguments.length > 3 ? e.call(arguments, 2) : r),
      typeof t == `function` && t.defaultProps != null)
    )
      for (o in t.defaultProps) s[o] === void 0 && (s[o] = t.defaultProps[o]);
    return x(t, s, i, a, null);
  }
  function x(e, r, i, a, o) {
    var s = {
      type: e,
      props: r,
      key: i,
      ref: a,
      __k: null,
      __: null,
      __b: 0,
      __e: null,
      __c: null,
      constructor: void 0,
      __v: o ?? ++n,
      __i: -1,
      __u: 0,
    };
    return (o == null && t.vnode != null && t.vnode(s), s);
  }
  function S(e) {
    return e.children;
  }
  function C(e, t) {
    ((this.props = e), (this.context = t));
  }
  function w(e, t) {
    if (t == null) return e.__ ? w(e.__, e.__i + 1) : null;
    for (var n; t < e.__k.length; t++)
      if ((n = e.__k[t]) != null && n.__e != null) return n.__e;
    return typeof e.type == `function` ? w(e) : null;
  }
  function T(e) {
    if (e.__P && e.__d) {
      var n = e.__v,
        r = n.__e,
        i = [],
        a = [],
        o = v({}, n);
      ((o.__v = n.__v + 1),
        t.vnode && t.vnode(o),
        I(
          e.__P,
          o,
          n,
          e.__n,
          e.__P.namespaceURI,
          32 & n.__u ? [r] : null,
          i,
          r ?? w(n),
          !!(32 & n.__u),
          a,
        ),
        (o.__v = n.__v),
        (o.__.__k[o.__i] = o),
        te(i, o, a),
        (n.__e = n.__ = null),
        o.__e != r && E(o));
    }
  }
  function E(e) {
    if ((e = e.__) != null && e.__c != null)
      return (
        (e.__e = e.__c.base = null),
        e.__k.some(function (t) {
          if (t != null && t.__e != null) return (e.__e = e.__c.base = t.__e);
        }),
        E(e)
      );
  }
  function D(e) {
    ((!e.__d && (e.__d = !0) && r.push(e) && !O.__r++) ||
      i != t.debounceRendering) &&
      ((i = t.debounceRendering) || a)(O);
  }
  function O() {
    try {
      for (var e, t = 1; r.length;)
        (r.length > t && r.sort(o), (e = r.shift()), (t = r.length), T(e));
    } finally {
      r.length = O.__r = 0;
    }
  }
  function k(e, t, n, r, i, a, o, s, c, l, u) {
    var d,
      f,
      p,
      g,
      _,
      v,
      y = (r && r.__k) || h,
      b = t.length;
    for (c = A(n, t, y, c, b), d = 0; d < b; d++)
      (p = n.__k[d]) != null &&
        ((f = (p.__i != -1 && y[p.__i]) || m),
        (p.__i = d),
        (v = I(e, p, f, i, a, o, s, c, l, u)),
        (g = p.__e),
        p.ref &&
          f.ref != p.ref &&
          (f.ref && L(f.ref, null, p), u.push(p.ref, p.__c || g, p)),
        _ == null && g != null && (_ = g),
        4 & p.__u
          ? ((c = j(p, c, e)), f.__e && (f.__e = null))
          : typeof p.type == `function` && v !== void 0
            ? (c = v)
            : g && (c = g.nextSibling),
        (p.__u &= -7));
    return ((n.__e = _), c);
  }
  function A(e, t, n, r, i) {
    var a,
      o,
      s,
      c,
      l,
      u = n.length,
      d = u,
      f = 0;
    for (e.__k = Array(i), a = 0; a < i; a++)
      (o = t[a]) != null && typeof o != `boolean` && typeof o != `function`
        ? (typeof o == `string` ||
          typeof o == `number` ||
          typeof o == `bigint` ||
          o.constructor == String
            ? (o = e.__k[a] = x(null, o, null, null, null))
            : _(o)
              ? (o = e.__k[a] = x(S, { children: o }, null, null, null))
              : o.constructor === void 0 && o.__b > 0
                ? (o = e.__k[a] =
                    x(o.type, o.props, o.key, o.ref ? o.ref : null, o.__v))
                : (e.__k[a] = o),
          (c = a + f),
          (o.__ = e),
          (o.__b = e.__b + 1),
          (s = null),
          (l = o.__i = M(o, n, c, d)) != -1 &&
            (d--, (s = n[l]) && (s.__u |= 2)),
          s == null || s.__v == null
            ? (l == -1 && (i > u ? f-- : i < u && f++),
              typeof o.type != `function` && (o.__u |= 4))
            : l != c &&
              (l == c - 1
                ? f--
                : l == c + 1
                  ? f++
                  : (l > c ? f-- : f++, (o.__u |= 4))))
        : (e.__k[a] = null);
    if (d)
      for (a = 0; a < u; a++)
        (s = n[a]) != null &&
          !(2 & s.__u) &&
          (s.__e == r && (r = w(s)), ie(s, s));
    return r;
  }
  function j(e, t, n) {
    var r, i;
    if (typeof e.type == `function`) {
      for (r = e.__k, i = 0; r && i < r.length; i++)
        r[i] && ((r[i].__ = e), (t = j(r[i], t, n)));
      return t;
    }
    e.__e != t &&
      (t && e.type && !t.parentNode && (t = w(e)),
      (t = n.insertBefore(e.__e, t || null)));
    do t &&= t.nextSibling;
    while (t != null && t.nodeType == 8);
    return t;
  }
  function M(e, t, n, r) {
    var i,
      a,
      o,
      s = e.key,
      c = e.type,
      l = t[n],
      u = l != null && !(2 & l.__u);
    if ((l === null && s == null) || (u && s == l.key && c == l.type)) return n;
    if (r > +!!u) {
      for (i = n - 1, a = n + 1; i >= 0 || a < t.length;)
        if (
          (l = t[(o = i >= 0 ? i-- : a++)]) != null &&
          !(2 & l.__u) &&
          s == l.key &&
          c == l.type
        )
          return o;
    }
    return -1;
  }
  function N(e, t, n) {
    t[0] == `-`
      ? e.setProperty(t, n ?? ``)
      : (e[t] =
          n == null ? `` : typeof n != `number` || g.test(t) ? n : n + `px`);
  }
  function P(e, t, n, r, i) {
    var a, o;
    n: if (t == `style`) {
      if (typeof n == `string`) e.style.cssText = n;
      else {
        if ((typeof r == `string` && (e.style.cssText = r = ``), r))
          for (t in r) (n && t in n) || N(e.style, t, ``);
        if (n) for (t in n) (r && n[t] == r[t]) || N(e.style, t, n[t]);
      }
    } else if (t[0] == `o` && t[1] == `n`)
      ((a = t != (t = t.replace(u, `$1`))),
        (o = t.toLowerCase()),
        (t =
          o in e || t == `onFocusOut` || t == `onFocusIn`
            ? o.slice(2)
            : t.slice(2)),
        (e.l ||= {}),
        (e.l[t + a] = n),
        n
          ? r
            ? (n[l] = r[l])
            : ((n[l] = d), e.addEventListener(t, a ? p : f, a))
          : e.removeEventListener(t, a ? p : f, a));
    else {
      if (i == `http://www.w3.org/2000/svg`)
        t = t.replace(/xlink(H|:h)/, `h`).replace(/sName$/, `s`);
      else if (
        t != `width` &&
        t != `height` &&
        t != `href` &&
        t != `list` &&
        t != `form` &&
        t != `tabIndex` &&
        t != `download` &&
        t != `rowSpan` &&
        t != `colSpan` &&
        t != `role` &&
        t != `popover` &&
        t in e
      )
        try {
          e[t] = n ?? ``;
          break n;
        } catch {}
      typeof n == `function` ||
        (n == null || (!1 === n && t[4] != `-`)
          ? e.removeAttribute(t)
          : e.setAttribute(t, t == `popover` && n == 1 ? `` : n));
    }
  }
  function F(e) {
    return function (n) {
      if (this.l) {
        var r = this.l[n.type + e];
        if (n[c] == null) n[c] = d++;
        else if (n[c] < r[l]) return;
        return r(t.event ? t.event(n) : n);
      }
    };
  }
  function I(e, n, r, i, a, o, s, c, l, u) {
    var d,
      f,
      p,
      m,
      g,
      b,
      x,
      T,
      E,
      D,
      O,
      A,
      j,
      M,
      N,
      P,
      F = n.type;
    if (n.constructor !== void 0) return null;
    (128 & r.__u && ((l = !!(32 & r.__u)), (o = [(c = n.__e = r.__e)])),
      (d = t.__b) && d(n));
    n: if (typeof F == `function`) {
      f = s.length;
      try {
        if (
          ((E = n.props),
          (D = F.prototype && F.prototype.render),
          (O = (d = F.contextType) && i[d.__c]),
          (A = d ? (O ? O.props.value : d.__) : i),
          r.__c
            ? (T = (p = n.__c = r.__c).__ = p.__E)
            : (D
                ? (n.__c = p = new F(E, A))
                : ((n.__c = p = new C(E, A)),
                  (p.constructor = F),
                  (p.render = ae)),
              O && O.sub(p),
              p.state || (p.state = {}),
              (p.__n = i),
              (m = p.__d = !0),
              (p.__h = []),
              (p._sb = [])),
          D && p.__s == null && (p.__s = p.state),
          D &&
            F.getDerivedStateFromProps != null &&
            (p.__s == p.state && (p.__s = v({}, p.__s)),
            v(p.__s, F.getDerivedStateFromProps(E, p.__s))),
          (g = p.props),
          (b = p.state),
          (p.__v = n),
          m)
        )
          (D &&
            F.getDerivedStateFromProps == null &&
            p.componentWillMount != null &&
            p.componentWillMount(),
            D &&
              p.componentDidMount != null &&
              p.__h.push(p.componentDidMount));
        else {
          if (
            (D &&
              F.getDerivedStateFromProps == null &&
              E !== g &&
              p.componentWillReceiveProps != null &&
              p.componentWillReceiveProps(E, A),
            n.__v == r.__v ||
              (!p.__e &&
                p.shouldComponentUpdate != null &&
                !1 === p.shouldComponentUpdate(E, p.__s, A)))
          ) {
            (n.__v != r.__v && ((p.props = E), (p.state = p.__s), (p.__d = !1)),
              (n.__e = r.__e),
              (n.__k = r.__k),
              n.__k.some(function (e) {
                e && (e.__ = n);
              }),
              h.push.apply(p.__h, p._sb),
              (p._sb = []),
              p.__h.length && s.push(p),
              (c = w(r)));
            break n;
          }
          (p.componentWillUpdate != null && p.componentWillUpdate(E, p.__s, A),
            D &&
              p.componentDidUpdate != null &&
              p.__h.push(function () {
                p.componentDidUpdate(g, b, x);
              }));
        }
        if (
          ((p.context = A),
          (p.props = E),
          (p.__P = e),
          (p.__e = !1),
          (j = t.__r),
          (M = 0),
          D)
        )
          ((p.state = p.__s),
            (p.__d = !1),
            j && j(n),
            (d = p.render(p.props, p.state, p.context)),
            h.push.apply(p.__h, p._sb),
            (p._sb = []));
        else
          do
            ((p.__d = !1),
              j && j(n),
              (d = p.render(p.props, p.state, p.context)),
              (p.state = p.__s));
          while (p.__d && ++M < 25);
        ((p.state = p.__s),
          p.getChildContext != null && (i = v(v({}, i), p.getChildContext())),
          D &&
            !m &&
            p.getSnapshotBeforeUpdate != null &&
            (x = p.getSnapshotBeforeUpdate(g, b)),
          (N =
            d != null && d.type === S && d.key == null
              ? ne(d.props.children)
              : d),
          (c = k(e, _(N) ? N : [N], n, r, i, a, o, s, c, l, u)),
          (p.base = n.__e),
          (n.__u &= -161),
          p.__h.length && s.push(p),
          T && (p.__E = p.__ = null));
      } catch (e) {
        if (((s.length = f), (n.__v = null), l || o != null)) {
          if (e.then) {
            for (n.__u |= l ? 160 : 128; c && c.nodeType == 8 && c.nextSibling;)
              c = c.nextSibling;
            (o != null && (o[o.indexOf(c)] = null), (n.__e = c));
          } else if (o != null) for (P = o.length; P--;) y(o[P]);
        } else n.__e = r.__e;
        ((n.__k ??= r.__k || []), e.then || ee(n), t.__e(e, n, r));
      }
    } else
      o == null && n.__v == r.__v
        ? ((n.__k = r.__k), (n.__e = r.__e))
        : (c = n.__e = re(r.__e, n, r, i, a, o, s, l, u));
    return ((d = t.diffed) && d(n), 128 & n.__u ? void 0 : c);
  }
  function ee(e) {
    e && (e.__c && (e.__c.__e = !0), e.__k && e.__k.some(ee));
  }
  function te(e, n, r) {
    for (var i = 0; i < r.length; i++) L(r[i], r[++i], r[++i]);
    (t.__c && t.__c(n, e),
      e.some(function (n) {
        try {
          ((e = n.__h),
            (n.__h = []),
            e.some(function (e) {
              e.call(n);
            }));
        } catch (e) {
          t.__e(e, n.__v);
        }
      }));
  }
  function ne(e) {
    return typeof e != `object` || !e || e.__b > 0
      ? e
      : _(e)
        ? e.map(ne)
        : e.constructor === void 0
          ? v({}, e)
          : null;
  }
  function re(n, r, i, a, o, s, c, l, u) {
    var d,
      f,
      p,
      h,
      g,
      v,
      b,
      x = i.props || m,
      S = r.props,
      C = r.type;
    if (
      (C == `svg`
        ? (o = `http://www.w3.org/2000/svg`)
        : C == `math`
          ? (o = `http://www.w3.org/1998/Math/MathML`)
          : (o ||= `http://www.w3.org/1999/xhtml`),
      s != null)
    ) {
      for (d = 0; d < s.length; d++)
        if (
          (g = s[d]) &&
          `setAttribute` in g == !!C &&
          (C ? g.localName == C : g.nodeType == 3)
        ) {
          ((n = g), (s[d] = null));
          break;
        }
    }
    if (n == null) {
      if (C == null) return document.createTextNode(S);
      ((n = document.createElementNS(o, C, S.is && S)),
        (l &&= (t.__m && t.__m(r, s), !1)),
        (s = null));
    }
    if (C == null) x === S || (l && n.data == S) || (n.data = S);
    else {
      if (
        ((s =
          C == `textarea` && S.defaultValue != null
            ? null
            : s && e.call(n.childNodes)),
        !l && s != null)
      )
        for (x = {}, d = 0; d < n.attributes.length; d++)
          x[(g = n.attributes[d]).name] = g.value;
      for (d in x)
        ((g = x[d]),
          d == `dangerouslySetInnerHTML`
            ? (p = g)
            : d == `children` ||
              d in S ||
              (d == `value` && `defaultValue` in S) ||
              (d == `checked` && `defaultChecked` in S) ||
              P(n, d, null, g, o));
      for (d in S)
        ((g = S[d]),
          d == `children`
            ? (h = g)
            : d == `dangerouslySetInnerHTML`
              ? (f = g)
              : d == `value`
                ? (v = g)
                : d == `checked`
                  ? (b = g)
                  : (l && typeof g != `function`) ||
                    x[d] === g ||
                    P(n, d, g, x[d], o));
      if (f)
        (l ||
          (p && (f.__html == p.__html || f.__html == n.innerHTML)) ||
          (n.innerHTML = f.__html),
          (r.__k = []));
      else if (
        (p && (n.innerHTML = ``),
        k(
          r.type == `template` ? n.content : n,
          _(h) ? h : [h],
          r,
          i,
          a,
          C == `foreignObject` ? `http://www.w3.org/1999/xhtml` : o,
          s,
          c,
          s ? s[0] : i.__k && w(i, 0),
          l,
          u,
        ),
        s != null)
      )
        for (d = s.length; d--;) y(s[d]);
      (l && C != `textarea`) ||
        ((d = `value`),
        C == `progress` && v == null
          ? n.removeAttribute(`value`)
          : v != null &&
            (v !== n[d] ||
              (C == `progress` && !v) ||
              (C == `option` && v != x[d])) &&
            P(n, d, v, x[d], o),
        (d = `checked`),
        b != null && b != n[d] && P(n, d, b, x[d], o));
    }
    return n;
  }
  function L(e, n, r) {
    try {
      if (typeof e == `function`) {
        var i = typeof e.__u == `function`;
        (i && e.__u(), (i && n == null) || (e.__u = e(n)));
      } else e.current = n;
    } catch (e) {
      t.__e(e, r);
    }
  }
  function ie(e, n, r) {
    var i, a;
    if (
      (t.unmount && t.unmount(e),
      (i = e.ref) && ((i.current && i.current != e.__e) || L(i, null, n)),
      (i = e.__c) != null)
    ) {
      if (i.componentWillUnmount)
        try {
          i.componentWillUnmount();
        } catch (e) {
          t.__e(e, n);
        }
      i.base = i.__P = i.__n = null;
    }
    if ((i = e.__k))
      for (a = 0; a < i.length; a++)
        i[a] && ie(i[a], n, r || typeof e.type != `function`);
    (r || y(e.__e), (e.__c = e.__ = e.__e = void 0));
  }
  function ae(e, t, n) {
    return this.constructor(e, n);
  }
  function oe(n, r, i) {
    var a, o, s, c;
    (r == document && (r = document.documentElement),
      t.__ && t.__(n, r),
      (o = (a = typeof i == `function`) ? null : (i && i.__k) || r.__k),
      (s = []),
      (c = []),
      I(
        r,
        (n = ((!a && i) || r).__k = b(S, null, [n])),
        o || m,
        m,
        r.namespaceURI,
        !a && i ? [i] : o ? null : r.firstChild ? e.call(r.childNodes) : null,
        s,
        !a && i ? i : o ? o.__e : r.firstChild,
        a,
        c,
      ),
      te(s, n, c),
      (n.props.children = null));
  }
  ((e = h.slice),
    (t = {
      __e: function (e, t, n, r) {
        for (var i, a, o; (t = t.__);)
          if ((i = t.__c) && !i.__)
            try {
              if (
                ((a = i.constructor) &&
                  a.getDerivedStateFromError != null &&
                  (i.setState(a.getDerivedStateFromError(e)), (o = i.__d)),
                i.componentDidCatch != null &&
                  (i.componentDidCatch(e, r || {}), (o = i.__d)),
                o)
              )
                return (i.__E = i);
            } catch (t) {
              e = t;
            }
        throw e;
      },
    }),
    (n = 0),
    (C.prototype.setState = function (e, t) {
      var n =
        this.__s != null && this.__s != this.state
          ? this.__s
          : (this.__s = v({}, this.state));
      (typeof e == `function` && (e = e(v({}, n), this.props)),
        e && v(n, e),
        e != null && this.__v && (t && this._sb.push(t), D(this)));
    }),
    (C.prototype.forceUpdate = function (e) {
      this.__v && ((this.__e = !0), e && this.__h.push(e), D(this));
    }),
    (C.prototype.render = S),
    (r = []),
    (a =
      typeof Promise == `function`
        ? Promise.prototype.then.bind(Promise.resolve())
        : setTimeout),
    (o = function (e, t) {
      return e.__v.__b - t.__v.__b;
    }),
    (O.__r = 0),
    (s = Math.random().toString(8)),
    (c = `__d` + s),
    (l = `__a` + s),
    (u = /(PointerCapture)$|Capture$/i),
    (d = 0),
    (f = F(!1)),
    (p = F(!0)));
  var R,
    z,
    B,
    V,
    H = 0,
    U = [],
    W = t,
    G = W.__b,
    K = W.__r,
    q = W.diffed,
    se = W.__c,
    ce = W.unmount,
    le = W.__;
  function J(e, t) {
    (W.__h && W.__h(z, e, H || t), (H = 0));
    var n = z.__H || (z.__H = { __: [], __h: [] });
    return (e >= n.__.length && n.__.push({}), n.__[e]);
  }
  function Y(e) {
    return ((H = 1), ue(Q, e));
  }
  function ue(e, t, n) {
    var r = J(R++, 2);
    if (
      ((r.t = e),
      !r.__c &&
        ((r.__ = [
          n ? n(t) : Q(void 0, t),
          function (e) {
            var t = r.__N ? r.__N[0] : r.__[0],
              n = r.t(t, e);
            t !== n && ((r.__N = [n, r.__[1]]), r.__c.setState({}));
          },
        ]),
        (r.__c = z),
        !z.__f))
    ) {
      var i = function (e, t, n) {
        if (!r.__c.__H) return !0;
        var i = !1,
          o = r.__c.props !== e;
        if (
          (r.__c.__H.__.some(function (e) {
            if (e.__N) {
              i = !0;
              var t = e.__[0];
              ((e.__ = e.__N), (e.__N = void 0), t !== e.__[0] && (o = !0));
            }
          }),
          a)
        ) {
          var s = a.call(this, e, t, n);
          return i ? s || o : s;
        }
        return !i || o;
      };
      z.__f = !0;
      var a = z.shouldComponentUpdate,
        o = z.componentWillUpdate;
      ((z.componentWillUpdate = function (e, t, n) {
        if (this.__e) {
          var r = a;
          ((a = void 0), i(e, t, n), (a = r));
        }
        o && o.call(this, e, t, n);
      }),
        (z.shouldComponentUpdate = i));
    }
    return r.__N || r.__;
  }
  function de(e, t) {
    var n = J(R++, 3);
    !W.__s && _e(n.__H, t) && ((n.__ = e), (n.u = t), z.__H.__h.push(n));
  }
  function fe(e) {
    return (
      (H = 5),
      pe(function () {
        return { current: e };
      }, [])
    );
  }
  function pe(e, t) {
    var n = J(R++, 7);
    return (_e(n.__H, t) && ((n.__ = e()), (n.__H = t), (n.__h = e)), n.__);
  }
  function me() {
    for (var e; (e = U.shift());) {
      var t = e.__H;
      if (e.__P && t)
        try {
          (t.__h.some(X), t.__h.some(Z), (t.__h = []));
        } catch (n) {
          ((t.__h = []), W.__e(n, e.__v));
        }
    }
  }
  ((W.__b = function (e) {
    ((z = null), G && G(e));
  }),
    (W.__ = function (e, t) {
      (e && t.__k && t.__k.__m && (e.__m = t.__k.__m), le && le(e, t));
    }),
    (W.__r = function (e) {
      (K && K(e), (R = 0));
      var t = (z = e.__c).__H;
      (t &&
        (B === z
          ? ((t.__h = []),
            (z.__h = []),
            t.__.some(function (e) {
              (e.__N && (e.__ = e.__N), (e.u = e.__N = void 0));
            }))
          : (t.__h.some(X), t.__h.some(Z), (t.__h = []), (R = 0))),
        (B = z));
    }),
    (W.diffed = function (e) {
      q && q(e);
      var t = e.__c;
      (t &&
        t.__H &&
        (t.__H.__h.length &&
          ((U.push(t) !== 1 && V === W.requestAnimationFrame) ||
            ((V = W.requestAnimationFrame) || ge)(me)),
        t.__H.__.some(function (e) {
          e.u &&= ((e.__H = e.u), void 0);
        })),
        (B = z = null));
    }),
    (W.__c = function (e, t) {
      (t.some(function (e) {
        try {
          (e.__h.some(X),
            (e.__h = e.__h.filter(function (e) {
              return !e.__ || Z(e);
            })));
        } catch (n) {
          (t.some(function (e) {
            e.__h &&= [];
          }),
            (t = []),
            W.__e(n, e.__v));
        }
      }),
        se && se(e, t));
    }),
    (W.unmount = function (e) {
      ce && ce(e);
      var t,
        n = e.__c;
      n &&
        n.__H &&
        (n.__H.__.some(function (e) {
          try {
            X(e);
          } catch (e) {
            t = e;
          }
        }),
        (n.__H = void 0),
        t && W.__e(t, n.__v));
    }));
  var he = typeof requestAnimationFrame == `function`;
  function ge(e) {
    var t,
      n = function () {
        (clearTimeout(r), he && cancelAnimationFrame(t), setTimeout(e));
      },
      r = setTimeout(n, 35);
    he && (t = requestAnimationFrame(n));
  }
  function X(e) {
    var t = z,
      n = e.__c;
    (typeof n == `function` && ((e.__c = void 0), n()), (z = t));
  }
  function Z(e) {
    var t = z;
    ((e.__c = e.__()), (z = t));
  }
  function _e(e, t) {
    return (
      !e ||
      e.length !== t.length ||
      t.some(function (t, n) {
        return t !== e[n];
      })
    );
  }
  function Q(e, t) {
    return typeof t == `function` ? t(e) : t;
  }
  var ve = 0;
  Array.isArray;
  function $(e, n, r, i, a, o) {
    n ||= {};
    var s,
      c,
      l = n;
    if (`ref` in l)
      for (c in ((l = {}), n)) c == `ref` ? (s = n[c]) : (l[c] = n[c]);
    var u = {
      type: e,
      props: l,
      key: r,
      ref: s,
      __k: null,
      __: null,
      __b: 0,
      __e: null,
      __c: null,
      constructor: void 0,
      __v: --ve,
      __i: -1,
      __u: 0,
      __source: a,
      __self: o,
    };
    if (typeof e == `function` && (s = e.defaultProps))
      for (c in s) l[c] === void 0 && (l[c] = s[c]);
    return (t.vnode && t.vnode(u), u);
  }
  function ye({ config: e }) {
    let [t, n] = Y(!1),
      [r, i] = Y([{ id: `1`, role: `assistant`, content: e.greeting }]),
      [a, o] = Y(``),
      [s, c] = Y(!1),
      l = fe(null),
      u = () => {
        l.current?.scrollIntoView({ behavior: `smooth` });
      };
    de(() => {
      u();
    }, [r]);
    let d = async (t) => {
        if ((t.preventDefault(), !a.trim() || s)) return;
        let n = { id: Date.now().toString(), role: `user`, content: a };
        (i((e) => [...e, n]), o(``), c(!0));
        let r = (Date.now() + 1).toString();
        i((e) => [...e, { id: r, role: `assistant`, content: `` }]);
        try {
          let t = await fetch(`http://localhost:3001/v1/chat`, {
            method: `POST`,
            headers: { "Content-Type": `application/json` },
            body: JSON.stringify({
              org_id: e.orgId,
              visitor_id: `visitor_` + Math.random().toString(36).substr(2, 9),
              query: n.content,
            }),
          });
          if (!t.ok) throw Error(`API Error`);
          let a = t.body?.getReader(),
            o = new TextDecoder();
          if (a) {
            let e = !1;
            for (; !e;) {
              let { value: t, done: n } = await a.read();
              if (((e = n), t)) {
                let e = o.decode(t).split(`
`);
                for (let t of e)
                  if (t.startsWith(`data: `))
                    try {
                      let e = JSON.parse(t.substring(6));
                      (e.token &&
                        i((t) =>
                          t.map((t) =>
                            t.id === r
                              ? { ...t, content: t.content + e.token }
                              : t,
                          ),
                        ),
                        e.metadata?.citations &&
                          i((t) =>
                            t.map((t) =>
                              t.id === r
                                ? { ...t, citations: e.metadata.citations }
                                : t,
                            ),
                          ));
                    } catch (e) {
                      console.error(`Error parsing SSE`, e);
                    }
              }
            }
          }
        } catch {
          i((e) =>
            e.map((e) =>
              e.id === r
                ? {
                    ...e,
                    content: `Sorry, I am having trouble connecting right now.`,
                  }
                : e,
            ),
          );
        } finally {
          c(!1);
        }
      },
      f = (e) =>
        e
          .split(/(\[\d+\])/g)
          .map((e, t) =>
            e.match(/^\[\d+\]$/)
              ? $(
                  `span`,
                  {
                    className: `aw-citation`,
                    title: `View Source`,
                    children: e,
                  },
                  t,
                )
              : $(`span`, { children: e }, t),
          );
    return $(`div`, {
      className: `aw-widget-container ${e.position}`,
      style: { "--theme-color": e.color },
      children: [
        $(`div`, {
          className: `aw-chat-window ${t ? `aw-open` : ``}`,
          children: [
            $(`div`, {
              className: `aw-header`,
              style: { background: e.color },
              children: [
                $(`span`, { children: `AI Support` }),
                $(`button`, {
                  className: `aw-close-btn`,
                  onClick: () => n(!1),
                  children: `✕`,
                }),
              ],
            }),
            $(`div`, {
              className: `aw-messages`,
              children: [
                r.map((t) =>
                  $(
                    `div`,
                    {
                      className: `aw-message aw-${t.role}`,
                      style:
                        t.role === `assistant` ? { background: e.color } : {},
                      children: f(t.content),
                    },
                    t.id,
                  ),
                ),
                s &&
                  $(`div`, {
                    className: `aw-message aw-assistant`,
                    style: { background: e.color, opacity: 0.7 },
                    children: $(`span`, {
                      className: `typing-dot`,
                      children: `...`,
                    }),
                  }),
                $(`div`, { ref: l }),
              ],
            }),
            $(`form`, {
              className: `aw-input-area`,
              onSubmit: d,
              children: [
                $(`input`, {
                  type: `text`,
                  className: `aw-input`,
                  placeholder: `Type your message...`,
                  value: a,
                  onInput: (e) => o(e.target.value),
                  disabled: s,
                }),
                $(`button`, {
                  type: `submit`,
                  className: `aw-send-btn`,
                  disabled: s || !a.trim(),
                  children: $(`svg`, {
                    viewBox: `0 0 24 24`,
                    children: $(`path`, {
                      d: `M2.01 21L23 12 2.01 3 2 10l15 2-15 2z`,
                    }),
                  }),
                }),
              ],
            }),
          ],
        }),
        !t &&
          $(`button`, {
            className: `aw-launcher`,
            style: { background: e.color },
            onClick: () => n(!0),
            children: $(`svg`, {
              viewBox: `0 0 24 24`,
              children: $(`path`, {
                d: `M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z`,
              }),
            }),
          }),
      ],
    });
  }
  var be = `:host{all:initial;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Helvetica,Arial,sans-serif}.aw-widget-container{z-index:999999;flex-direction:column;display:flex;position:fixed}.aw-widget-container.bottom-right{align-items:flex-end;bottom:24px;right:24px}.aw-widget-container.bottom-left{align-items:flex-start;bottom:24px;left:24px}.aw-launcher{cursor:pointer;border:none;border-radius:30px;justify-content:center;align-items:center;width:60px;height:60px;transition:transform .2s;display:flex;position:relative;box-shadow:0 10px 15px -3px #0000001a,0 4px 6px -2px #0000000d}.aw-launcher:hover{transform:scale(1.05)}.aw-launcher svg{fill:#fff;width:28px;height:28px}.aw-chat-window{opacity:0;pointer-events:none;background:#fff;border-radius:16px;flex-direction:column;width:380px;height:600px;max-height:calc(100vh - 120px);margin-bottom:16px;transition:all .3s cubic-bezier(.16,1,.3,1);display:flex;overflow:hidden;transform:translateY(20px);box-shadow:0 20px 25px -5px #0000001a,0 10px 10px -5px #0000000a}.aw-chat-window.aw-open{opacity:1;pointer-events:all;transform:translateY(0)}.aw-header{color:#fff;justify-content:space-between;align-items:center;padding:16px 20px;font-size:16px;font-weight:600;display:flex}.aw-close-btn{color:#fff;cursor:pointer;opacity:.8;background:0 0;border:none}.aw-close-btn:hover{opacity:1}.aw-messages{background:#f9fafb;flex-direction:column;flex:1;gap:16px;padding:20px;display:flex;overflow-y:auto}.aw-message{border-radius:12px;max-width:85%;padding:12px 16px;font-size:14px;line-height:1.5}.aw-message.aw-user{color:#111827;background:#e5e7eb;border-bottom-right-radius:4px;align-self:flex-end}.aw-message.aw-assistant{color:#fff;border-bottom-left-radius:4px;align-self:flex-start}.aw-input-area{background:#fff;border-top:1px solid #e5e7eb;gap:8px;padding:16px;display:flex}.aw-input{border:1px solid #d1d5db;border-radius:20px;outline:none;flex:1;padding:10px 16px;font-family:inherit;font-size:14px}.aw-input:focus{border-color:var(--theme-color)}.aw-send-btn{background:var(--theme-color);cursor:pointer;border:none;border-radius:20px;justify-content:center;align-items:center;width:40px;height:40px;display:flex}.aw-send-btn svg{fill:#fff;width:20px;height:20px}.aw-send-btn:disabled{opacity:.5;cursor:not-allowed}.aw-citation{color:#fff;cursor:pointer;vertical-align:super;background:#fff3;border-radius:10px;margin-left:4px;padding:2px 6px;font-size:10px;display:inline-block}.aw-citation:hover{background:#fff6}`;
  function xe() {
    let e = document.currentScript,
      t = {
        color: e?.getAttribute(`data-color`) || `#4F46E5`,
        position: e?.getAttribute(`data-position`) || `bottom-right`,
        greeting:
          e?.getAttribute(`data-greeting`) ||
          `Hi there! How can I help you today?`,
        orgId: e?.getAttribute(`data-org-id`) || ``,
      },
      n = document.createElement(`div`);
    ((n.id = `ai-workforce-widget-root`), document.body.appendChild(n));
    let r = n.attachShadow({ mode: `open` }),
      i = document.createElement(`style`);
    ((i.textContent = be), r.appendChild(i));
    let a = document.createElement(`div`);
    (r.appendChild(a), oe($(ye, { config: t }), a));
  }
  typeof window < `u` &&
    (document.readyState === `complete`
      ? xe()
      : window.addEventListener(`load`, xe));
})();
