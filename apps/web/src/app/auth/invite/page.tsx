  ...
  <>
    {[["tl","1px 0 0 1px","0","0","auto","auto"],
      ...
      <span key={k} style={{
        ...
        borderColor: T.g, borderStyle:"solid", borderWidth: bw as number | string, opacity: 0.5,
...
function InviteAcceptanceInner() {
  ...
  const handleAccept = async () => {
    ...
    if (!user) {
      ...
      router.push(`/signup?email=${encodeURIComponent(invitedEmail)}&invite_token=${token}`);
      return;
    }
  ...
  if (status === "loading" || userLoading) {
    return (
      <div style={{ minHeight: "100vh", background: T.bg, color: T.text, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ fontFamily: T.mono, color: T.g }}>VALIDATING INVITATION...</div>
      </div>
    );
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        width: "100%", maxWidth: 460, padding: "3rem", textAlign: "center",
        ...
        <div style={{ 
          width: 80, height: 80, border: `1px solid ${T.border}`, background: "rgba(0,255,136,0.02)",
        ...
        {status === "invalid" && (
          <>
            <div style={{ fontFamily:T.display, fontSize:"1.5rem", fontWeight:700, color:T.warn, marginBottom:"1rem" }}>
              Invitation Error
            </div>
            <p style={{ color: "rgba(200,255,232,0.6)", fontSize: "0.95rem", lineHeight: 1.6, marginBottom: "2rem" }}>
              {errorMsg}
            </p>
            <Link href="/auth/login" style={{
              display: "block", width:"100%", fontFamily:T.mono, fontSize:"0.85rem", letterSpacing:"0.1em", textTransform:"uppercase", textDecoration: "none",
              color:T.bg, background:T.warn, border:"none", padding:"1rem", cursor:"pointer",
        ...
        {status === "accepted" && (
          <>
            <div style={{ fontFamily:T.display, fontSize:"1.5rem", fontWeight:700, color:T.g, marginBottom:"1rem" }}>
              Access Granted
            </div>
            <p style={{ color: "rgba(200,255,232,0.6)", fontSize: "0.95rem", lineHeight: 1.6, marginBottom: "2rem" }}>
              You have successfully joined <strong>{orgName}</strong>. Redirecting to your dashboard...
            </p>
          </>
        )}
        ...
        {status === "valid" && (
          <>
            <div style={{ fontFamily:T.display, fontSize:"1.8rem", fontWeight:700, color:"#fff", marginBottom:"0.5rem" }}>
              You&apos;ve been invited!
            </div>
            
            <p style={{ color: "rgba(200,255,232,0.6)", fontSize: "0.95rem", lineHeight: 1.6, marginBottom: "2.5rem" }}>
              You have been granted access to join <strong style={{ color: "#fff" }}>{orgName}</strong> on the AI Workforce platform as <strong style={{ color: T.g2 }}>{invitedEmail}</strong>.
            </p>

            {user ? (
              user.email.toLowerCase() === invitedEmail.toLowerCase() ? (
                <button onClick={handleAccept} disabled={isAccepting} style={{
                  width:"100%", fontFamily:T.mono, fontSize:"0.85rem", letterSpacing:"0.1em", textTransform:"uppercase",
                  color:T.bg, background:T.g, border:"none", padding:"1rem", cursor: isAccepting ? "wait" : "pointer", marginBottom:"1rem",
                  clipPath:"polygon(12px 0%,100% 0%,calc(100% - 12px) 100%,0% 100%)",
                  transition:"box-shadow 0.2s, background 0.2s", opacity: isAccepting ? 0.7 : 1
                }}>
                  {isAccepting ? "ACCEPTING..." : "▶ ACCEPT INVITATION"}
                </button>
              ) : (
                <div style={{ marginBottom: "1rem" }}>
                  <p style={{ color: T.warn, fontSize: "0.85rem", marginBottom: "1rem" }}>
              ...
              <button onClick={handleAccept} style={{
                width:"100%", fontFamily:T.mono, fontSize:"0.85rem", letterSpacing:"0.1em", textTransform:"uppercase",
                color:T.bg, background:T.g, border:"none", padding:"1rem", cursor:"pointer", marginBottom:"1rem",
                clipPath:"polygon(12px 0%,100% 0%,calc(100% - 12px) 100%,0% 100%)",
            ...
            {!user && (
              <div style={{ fontFamily:T.mono, fontSize:"0.7rem", color:"rgba(200,255,232,0.35)", textAlign:"center", marginTop:"1.5rem" }}>
                ...
                <Link href={`/auth/login?redirect=${encodeURIComponent("/auth/invite?token=" + token)}`} style={{ color:T.g2, textDecoration:"none", transition:"text-shadow 0.2s" }}
...
export default function InviteAcceptancePage() {
  return (
    <Suspense fallback={
      <div style={{ minHeight: "100vh", background: T.bg, color: T.text, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ fontFamily: T.mono, color: T.g }}>LOADING INVITATION...</div>
      </div>
    }>
