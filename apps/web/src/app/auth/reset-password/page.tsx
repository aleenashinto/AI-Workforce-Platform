    ...
    <div style={{
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit}>
          <ModalField label="New Password" type="password" placeholder="••••••••••••" value={formData.password} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({...formData, password: e.target.value})} />
          
          <div style={{ marginBottom: "1.5rem", background: "rgba(0,0,0,0.2)", padding: "1rem", borderRadius: "4px", border: `1px solid rgba(0,255,136,0.1)` }}>
            <div style={{ fontFamily: T.mono, fontSize: "0.65rem", textTransform: "uppercase", color: T.muted, marginBottom: "0.8rem" }}>Password Requirements:</div>
            <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: "0.4rem" }}>
              <li style={{ display: "flex", alignItems: "center", gap: "0.5rem", fontSize: "0.8rem", color: reqs.length ? T.g : "rgba(200,255,232,0.4)" }}>
                <span style={{ fontFamily: T.mono }}>{reqs.length ? "✓" : "○"}</span> At least 8 characters
              </li>
              <li style={{ display: "flex", alignItems: "center", gap: "0.5rem", fontSize: "0.8rem", color: reqs.upper ? T.g : "rgba(200,255,232,0.4)" }}>
                <span style={{ fontFamily: T.mono }}>{reqs.upper ? "✓" : "○"}</span> One uppercase letter
              </li>
              <li style={{ display: "flex", alignItems: "center", gap: "0.5rem", fontSize: "0.8rem", color: reqs.number ? T.g : "rgba(200,255,232,0.4)" }}>
                <span style={{ fontFamily: T.mono }}>{reqs.number ? "✓" : "○"}</span> One number
              </li>
            </ul>
          </div>
          ...
          <button type="submit" disabled={!allReqsMet || !match} style={{
            ...
            border:"none", padding:"1rem", cursor: (!allReqsMet || !match) ? "not-allowed" : "pointer", 
            marginBottom:"1.5rem", marginTop: "0.5rem",
            clipPath:"polygon(12px 0%,100% 0%,calc(100% - 12px) 100%,0% 100%)",
            transition:"box-shadow 0.2s, background 0.2s",
          }}
            onMouseEnter={e=>{if(allReqsMet && match){e.currentTarget.style.boxShadow=T.glow;e.currentTarget.style.background="#fff";}}}
            onMouseLeave={e=>{if(allReqsMet && match){e.currentTarget.style.boxShadow="";e.currentTarget.style.background=T.g;}}}
