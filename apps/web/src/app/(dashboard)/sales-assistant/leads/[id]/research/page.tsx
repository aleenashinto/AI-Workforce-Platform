...
export default function ResearchBriefPage() {
  ...
  const fetchLead = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/v1/sales/leads/${leadId}`, { credentials: "include",
  ...
  const handleRegenerate = async () => {
    ...
    try {
      const token = await getToken();
      await fetch(`${process.env.NEXT_PUBLIC_API_URL}/v1/sales/leads/${leadId}/research`, { credentials: "include",
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` }
      });
