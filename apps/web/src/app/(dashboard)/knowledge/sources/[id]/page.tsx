...
export default function SourceDetailPage({ params }: { params: { id: string } }) {
  ...
  const logs = [
    ...
    { id: 2, time: '10:00:15 AM', message: 'File uploaded successfully', type: 'info' },
    { id: 3, time: '10:01:00 AM', message: 'Parsing started', type: 'info' },
    { id: 4, time: '10:01:30 AM', message: 'Parsing completed (45 pages)', type: 'info' },
    { id: 5, time: '10:02:00 AM', message: 'Embedding chunks generated', type: 'info' },
    { id: 6, time: '10:02:30 AM', message: 'Knowledge source ready', type: 'success' },
  ];
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center space-x-3 mb-1">
            <Link href="/knowledge/sources" className="text-gray-400 hover:text-white transition-colors">
              Sources
            </Link>
        ...
        <div className="flex space-x-3">
          <Button variant="outline" className="border-[#3F3F5A] hover:bg-[#2A2A3C] text-gray-200">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Card className="bg-[#2A2A3C] border-[#3F3F5A] text-white md:col-span-1">
          ...
          <CardContent className="space-y-4">
            ...
            {source.config && Object.entries(source.config).map(([k, v]) => (
              <div key={k}>
                <p className="text-sm text-gray-400 capitalize">{k.replace(/([A-Z])/g, ' $1').trim()}</p>
                <p className="font-medium mt-1 truncate" title={String(v)}>{String(v)}</p>
              </div>
            ))}
          </CardContent>
        </Card>
              ...
              {logs.map((log) => (
                <div key={log.id} className="flex items-start gap-4 text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {log.type === 'success' && <div className="w-2 h-2 rounded-full bg-emerald-400 mt-1.5" />}
                    {log.type === 'error' && <AlertTriangle className="w-4 h-4 text-rose-400" />}
                    <span className={
