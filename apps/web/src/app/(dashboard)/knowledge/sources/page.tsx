...
export default function SourcesPage() {
  ...
  const fetchSources = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/v1/sources?org_id=${currentOrgId}`, { credentials: "include",
  ...
  const handleAddSource = async () => {
    try {
      ...
      await fetch(`${process.env.NEXT_PUBLIC_API_URL}/v1/sources`, { credentials: "include",
        method: 'POST',
        headers: { 
