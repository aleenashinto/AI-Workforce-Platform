...
export default function ConversationDetailsPage({
  ...
  const fetchConversation = async () => {
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/agent/conversations/${params.id}`,
        {
          credentials: "include",
        ...
        setMessages([
          {
            ...
            role: "user",
            content:
              "Hi, I received my order but it is damaged. What should I do?",
            time: "10:00 AM",
          },
          {
            id: "2",
            role: "assistant",
            content:
              "I'm sorry to hear that your order arrived damaged. According to our policy, you can request a replacement or a full refund within 30 days of receipt.",
            time: "10:01 AM",
            confidence: 0.95,
          },
          {
            ...
            role: "system",
            content:
              "User sentiment is highly negative. Escalating to human agent.",
            time: "10:02 AM",
          },
          {
            id: "3",
            role: "user",
            content: "I want a refund. Can you process it now?",
            time: "10:05 AM",
          },
        ]);
  ...
  const handleSend = async () => {
    ...
    try {
      const token = await getToken();
      await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/agent/conversations/${params.id}/reply`,
        {
          credentials: "include",
          method: "POST",
          headers: {
  ...
  const requestCopilot = async () => {
    try {
      ...
      const token = await getToken();
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/agent/conversations/${params.id}/copilot`,
        {
          credentials: "include",
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
      ...
      if (json.success && json.suggestion) {
        ...
        setCopilotSuggestion(
          "I can certainly help you with that. To process the refund, I will need your order number. Could you please provide it?",
        );
      }
    } catch (e) {
      console.error(e);
        ...
        <div className="h-16 border-b border-[#3F3F5A] flex items-center justify-between px-6 bg-[#1E1E2E]">
          <div className="flex items-center gap-3">
            <h2 className="font-semibold text-white">
              Conversation #{params.id.substring(0, 8)}
            </h2>
          ...
          <div className="flex items-center gap-3">
            {!hasClaimed && (
              <Button
                onClick={handleClaim}
                className="bg-gradient-to-r from-[#D122E3] to-[#00F2FE] hover:opacity-90 h-8 text-sm text-white border-0"
              >
                Claim & Pause AI
              </Button>
            )}
            {hasClaimed && conversation?.status !== "resolved" && (
              <>
                ...
                <Button
                  ...
                  variant="outline"
                  className="border-emerald-500/50 text-emerald-400 hover:bg-emerald-500/10 h-8 text-sm"
                >
                ...
                >
                  ...
                  <div
                    className={`p-3.5 rounded-2xl shadow-sm ${
                      msg.role === "user"
                        ? "bg-[#2A2A3C] text-gray-200 border border-[#3F3F5A] rounded-tr-none"
                        : msg.role === "assistant"
                          ? "bg-[#1E1E2E] text-gray-200 border border-[#3F3F5A] rounded-tl-none"
        ...
        <div className="p-4 bg-[#1E1E2E] border-t border-[#3F3F5A] relative">
          ...
          {hasClaimed && isCopilotActive && (
            <div className="absolute bottom-full left-0 right-0 p-4 mb-2 animate-in slide-in-from-bottom-2">
              <div className="bg-[#1E1E2E] border border-[#D122E3]/50 rounded-xl p-4 shadow-[0_0_30px_rgba(209,34,227,0.15)] flex gap-4">
                <div className="mt-1">
                  <Bot className="w-5 h-5 text-[#D122E3]" />
                </div>
                <div className="flex-1">
                  <div className="text-xs font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#D122E3] to-[#00F2FE] uppercase tracking-wider mb-2">
                  ...
                  {copilotSuggestion === "Generating suggestion..." ? (
                    ...
                    <p className="text-sm text-gray-200 italic bg-[#1A1A24] p-3 rounded-lg border border-[#3F3F5A]">
                      "{copilotSuggestion}"
                    </p>
                  )}
                </div>
                <div className="flex flex-col gap-2 shrink-0">
                  <Button
                    ...
                    disabled={copilotSuggestion === "Generating suggestion..."}
                    className="bg-gradient-to-r from-[#D122E3] to-[#00F2FE] hover:opacity-90 h-8 text-xs px-4 text-white border-0"
                  >
                    Apply
                  </Button>
                  <Button
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              className="bg-[#2A2A3C] border border-[#3F3F5A] text-white hover:bg-[#3F3F5A] shrink-0"
            >
          ...
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-[#1E1E2E] border border-[#3F3F5A] rounded-xl w-full max-w-md p-6 shadow-2xl">
              ...
              <p className="text-gray-300 text-sm mb-6">
                ...
                tool, which requires agent confirmation.
              </p>
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          <div className="text-center">
            <h4 className="font-medium text-white text-lg">
              {conversation?.visitor_id || "visitor_492"}
            </h4>
        ...
        <div className="space-y-5 text-sm bg-[#1A1A24] p-4 rounded-xl border border-[#3F3F5A]">
          ...
          <div>
            ...
            <p className="text-gray-200 flex items-center">
              <Clock className="w-3.5 h-3.5 mr-1.5 text-amber-400" /> 4 mins
            </p>
          </div>
          <div>
            <p className="text-gray-500 text-xs mb-1 uppercase tracking-wider">
        ...
        {hasClaimed && conversation?.status !== "resolved" && (
          <div className="mt-auto pt-6">
            <Button
              ...
              className="w-full bg-gradient-to-r from-[#2A2A3C] to-[#2A2A3C] hover:from-[#D122E3]/20 hover:to-[#00F2FE]/20 border border-[#3F3F5A] hover:border-[#D122E3]/50 text-white shadow-lg transition-all"
            >
              <Bot className="w-4 h-4 mr-2 text-[#D122E3]" /> Ask Copilot for
              Help
            </Button>
          </div>
        )}
