...
export default function ConversationDetailsPage({ params }: { params: { id: string } }) {
  ...
  const fetchConversation = async () => {
    try {
      ...
      const res = await fetch(`http://localhost:3001/agent/conversations/${params.id}`, {
  ...
  const handleSend = async () => {
    ...
    try {
      const token = await getToken();
      await fetch(`http://localhost:3001/agent/conversations/${params.id}/reply`, {
        method: 'POST',
  ...
  const handleResolve = async () => {
    try {
      const token = await getToken();
      await fetch(`http://localhost:3001/agent/conversations/${params.id}`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
  ...
  const requestCopilot = async () => {
    try {
      ...
      const token = await getToken();
      const res = await fetch(`http://localhost:3001/agent/conversations/${params.id}/copilot`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` }
      });
      const json = await res.json();
