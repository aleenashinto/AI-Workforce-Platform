  ...
  const chatHistory = [
    {
      id: 1,
      role: "user",
      content: "I can't log in to my account. How do I reset my password?",
      time: "10:40 AM",
    },
    {
      id: 2,
      role: "assistant",
      content:
        "I can help with that! You can reset your password by going to the login page and clicking 'Forgot Password' [1]. Would you like me to send you the direct link?",
      time: "10:40 AM",
      confidence: 0.95,
    },
    {
      ...
      role: "user",
      content:
        "Yes, but my email is changed. I don't have access to the old one.",
      time: "10:41 AM",
    },
    {
      id: 4,
      role: "system",
      content:
        "Escalated to human due to low confidence (0.4) for account recovery.",
      time: "10:42 AM",
      isSystem: true,
    },
  ];
                ...
                <Badge
                  ...
                  className={
                    conv.status === "escalated"
                      ? "text-rose-400 border-rose-500/50 bg-rose-500/10 text-[10px]"
                      : conv.status === "active"
                        ? "text-blue-400 border-blue-500/50 bg-blue-500/10 text-[10px]"
      ...
      <div className="flex-1 flex flex-col bg-[#1E1E2E]">
        <div className="h-16 border-b border-[#3F3F5A] flex items-center justify-between px-6 bg-[#2A2A3C]">
              ...
              <p className="text-xs text-[color:var(--t-text)] flex items-center gap-2">
                ...
                {activeConv.status === "escalated" && (
                  <span className="text-rose-400 flex items-center">
                    <AlertTriangle className="w-3 h-3 ml-2 mr-1" /> Escalated
                  </span>
                )}
              </p>
            </div>
          ...
          <div>
            <Button
              variant="outline"
              className="border-[#3F3F5A] text-white hover:bg-[#3F3F5A]"
            >
              Resolve
            </Button>
          </div>
