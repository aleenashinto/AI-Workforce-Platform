...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
// eslint-disable-next-line @typescript-eslint/no-unused-vars
import { Search, Filter, MessageSquare, Clock, User, Reply, AlertTriangle, CheckCircle } from "lucide-react";
      ...
      <div style={{ width: 320, borderRight: `1px solid ${T.border}`, background: T.bg2, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          {loading ? (
            ...
            <div style={{ padding: "2rem", textAlign: "center", color: T.muted, fontFamily: T.mono, fontSize: "0.8rem" }}>NO_CONVERSATIONS_FOUND</div>
          ) : list.map((c, i) => (
            <Link key={c.id} href={`/customer-support/inbox/${c.id}`} style={{ textDecoration: "none" }}>
              <div style={{ padding: "1.2rem", borderBottom: `1px solid ${T.border}`, cursor: "pointer", transition: "background 0.2s", background: "transparent" }} onMouseEnter={e => e.currentTarget.style.background = "rgba(0,255,136,0.05)"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                ...
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  ...
                  {c.csat_score && (
                    <span style={{ fontFamily: T.mono, fontSize: "0.6rem", padding: "0.2rem 0.4rem", background: "rgba(0,207,255,0.1)", border: `1px solid rgba(0,207,255,0.3)`, color: T.g2 }}>CSAT: {c.csat_score}</span>
                  )}
                </div>
              </div>
            </Link>
