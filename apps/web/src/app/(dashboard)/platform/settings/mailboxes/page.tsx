...
export default function MailboxesPage() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/mailboxes`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const togglePause = async (id: string, currentlyPaused: boolean) => {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/mailboxes/${id}/pause`,
      {
        credentials: "include",
        method: "PATCH",
        headers: {
    ...
    if (res.ok) {
      setMailboxes(
        mailboxes.map((m) =>
          m.id === id
            ? { ...m, status: currentlyPaused ? "active" : "paused" }
            : m,
        ),
      );
    }
