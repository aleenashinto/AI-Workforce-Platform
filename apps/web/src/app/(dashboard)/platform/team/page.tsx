...
import { 
  Users, UserPlus, Mail, Shield, CheckCircle, X, Search, Filter
} from "lucide-react";
...
const ALL_ROLES: MemberRole[] = ['owner', 'admin', 'support_lead', 'support_agent', 'sales_lead', 'sales_rep', 'viewer'];
...
function RoleBadge({ role }: { role: string }) {
  ...
  return (
    <span style={{
      fontFamily: T.mono, fontSize: '0.65rem', color: c.color,
      background: c.bg, padding: '0.3rem 0.6rem',
      border: `1px solid ${c.border}`, display: 'inline-block', marginRight: '0.4rem',
    ...
    <div style={{ padding: "2rem", maxWidth: 1100, margin: "0 auto", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2.5rem" }}>
        ...
        {canManageMembers && (
          ...
          onMouseEnter={e => { e.currentTarget.style.background = "#fff"; e.currentTarget.style.boxShadow = `0 0 30px rgba(255,255,255,0.4)`; }}
          onMouseLeave={e => { e.currentTarget.style.background = T.g; e.currentTarget.style.boxShadow = T.glow; }}
          >
      ...
      <div style={{ display: "flex", gap: "1rem", marginBottom: "1.5rem" }}>
        <div style={{ flex: 1, position: "relative" }}>
          ...
          <input 
            ...
            style={{ 
              ...
              padding: "0.8rem 1rem 0.8rem 2.8rem", color: T.text, fontFamily: T.mono, fontSize: "0.85rem",
        ...
        <button style={{ 
          background: "rgba(0,255,136,0.05)", border: `1px solid ${T.border}`, padding: "0.8rem 1.2rem",
          color: T.g, fontFamily: T.mono, fontSize: "0.85rem", display: "flex", alignItems: "center", gap: "0.5rem",
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ borderBottom: `1px solid ${T.border}`, background: "rgba(0,0,0,0.2)" }}>
              <th style={{ textAlign: "left", padding: "1.2rem 1.5rem", fontFamily: T.mono, fontSize: "0.7rem", color: T.g, textTransform: "uppercase", letterSpacing: "0.1em" }}>User</th>
              <th style={{ textAlign: "left", padding: "1.2rem 1.5rem", fontFamily: T.mono, fontSize: "0.7rem", color: T.g, textTransform: "uppercase", letterSpacing: "0.1em", width: "40%" }}>Roles</th>
              <th style={{ textAlign: "left", padding: "1.2rem 1.5rem", fontFamily: T.mono, fontSize: "0.7rem", color: T.g, textTransform: "uppercase", letterSpacing: "0.1em" }}>Status</th>
              {canManageMembers && (
                <th style={{ textAlign: "right", padding: "1.2rem 1.5rem", fontFamily: T.mono, fontSize: "0.7rem", color: T.g, textTransform: "uppercase", letterSpacing: "0.1em" }}>Actions</th>
              )}
            </tr>
          </thead>
          <tbody>
            {members.map((m, i) => (
              <tr 
                ...
                style={{ 
                  borderBottom: i === members.length - 1 ? "none" : `1px solid rgba(0,255,136,0.05)`,
              ...
              >
                <td style={{ padding: "1.5rem" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                    <div style={{ 
                      width: 40, height: 40, borderRadius: "50%", background: `linear-gradient(135deg, ${T.g}22, ${T.g2}22)`,
                      border: `1px solid ${T.border}`, display: "flex", alignItems: "center", justifyContent: "center",
                      fontFamily: T.display, color: "#fff", fontSize: "1.2rem", fontWeight: 700
                    }}>
                      {m.name.charAt(0)}
                    </div>
                    <div>
                      <div style={{ fontFamily: T.body, fontSize: "1.1rem", color: "#fff", fontWeight: 600, marginBottom: "0.2rem", letterSpacing: "0.02em" }}>{m.name}</div>
                ...
                <td style={{ padding: "1.5rem" }}>
                  {editingIndex === i && canManageMembers ? (
                    <div style={{ 
                      background: "rgba(0,0,0,0.4)", border: `1px solid ${T.border}`, padding: "1rem", 
                      borderRadius: "4px", display: 'flex', flexWrap: 'wrap', gap: '0.4rem',
                      ...
                      {ALL_ROLES.map(role => {
                        ...
                        return (
                          <button
                            ...
                            onMouseEnter={e => { if(!assigned) e.currentTarget.style.background = 'rgba(255,255,255,0.08)'; }}
                            onMouseLeave={e => { if(!assigned) e.currentTarget.style.background = 'rgba(255,255,255,0.03)'; }}
                          >
                ...
                <td style={{ padding: "1.5rem" }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
                      {m.status === 'Active' ? (
                        <>
                          <div style={{ width: 8, height: 8, borderRadius: "50%", background: T.g, boxShadow: `0 0 10px ${T.g}` }} />
                          <span style={{ fontFamily: T.mono, fontSize: "0.8rem", color: T.g }}>ACTIVE</span>
                        </>
                      ) : (
                        <>
                          <div style={{ width: 8, height: 8, borderRadius: "50%", background: T.warn, boxShadow: `0 0 10px ${T.warn}` }} />
                          <span style={{ fontFamily: T.mono, fontSize: "0.8rem", color: T.warn }}>PENDING</span>
                        </>
                      )}
                    </div>
                    <div style={{ fontFamily: T.mono, fontSize: "0.65rem", color: T.muted }}>
                  ...
                  <td style={{ padding: "1.5rem", textAlign: "right" }}>
                    {editingIndex === i ? (
                      <button
                        ...
                        style={{ 
                          background: T.g, border: "none", color: T.bg, fontFamily: T.mono, fontSize: "0.7rem", 
                          padding: "0.5rem 1rem", cursor: "pointer", textTransform: "uppercase", 
                          display: 'inline-flex', alignItems: 'center', gap: '0.4rem', fontWeight: "bold",
                      ...
                      <button
                        ...
                        style={{ 
                          background: "rgba(0,255,136,0.05)", border: `1px solid ${T.border}`, color: T.text, 
                          fontFamily: T.mono, fontSize: "0.7rem", padding: "0.5rem 1rem", cursor: "pointer", 
