...
import { 
  LayoutDashboard, Inbox, Database, MessageSquare, LineChart, Settings, Bot, 
  Search, Users, Activity, FileText, Send, Mail, Briefcase, Puzzle, PieChart,
  LogOut, ChevronDown, ChevronRight, AlertTriangle, User
} from "lucide-react";
...
const navigation = [
  {
    ...
    items: [
      { label: "Overview", path: "/customer-support/overview", icon: LayoutDashboard },
      { label: "Inbox", path: "/customer-support/inbox", icon: Inbox },
      { label: "Conversations", path: "/customer-support/conversations", icon: MessageSquare },
  ...
  },
  {
    ...
    key: "sales-assistant",
    items: [
      { label: "Overview", path: "/sales-assistant/overview", icon: LayoutDashboard },
      { label: "ICP", path: "/sales-assistant/icp", icon: TargetIcon },
      { label: "Lead Discovery", path: "/sales-assistant/lead-discovery", icon: Search },
      { label: "Leads", path: "/sales-assistant/leads", icon: Users },
      { label: "Research", path: "/sales-assistant/research", icon: Search },
      { label: "Drafts", path: "/sales-assistant/drafts", icon: FileText },
      { label: "Sequences", path: "/sales-assistant/sequences", icon: Send },
      { label: "Mailboxes", path: "/sales-assistant/mailboxes", icon: Mail },
      { label: "Analytics", path: "/sales-assistant/analytics", icon: PieChart }
    ]
  },
  {
    title: "PLATFORM",
  ...
  useEffect(() => {
    ...
    setExpandedGroups(prev => {
      ...
      navigation.forEach(group => {
        const isActiveGroup = group.items.some(item => pathname.startsWith(item.path));
    ...
    >
      ...
      <div style={{ height: 80, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 1.5rem", borderBottom: `1px solid ${T.border}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.8rem" }}>
          <div style={{ width: 32, height: 32, background: "rgba(0,255,136,0.1)", border: `1px solid ${T.g}`, borderRadius: 4, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: T.glow }}>
            <Bot size={18} color={T.g} />
          </div>
          <div style={{ display: "flex", flexDirection: "column" }}>
            <span style={{ fontFamily: T.display, fontSize: "1.1rem", fontWeight: 700, color: "#fff", letterSpacing: "0.05em" }}>AI<span style={{color:T.g}}>WORKFORCE</span></span>
            <span style={{ fontFamily: T.mono, fontSize: "0.6rem", textTransform: "uppercase", letterSpacing: "0.15em", color: T.g }}>Enterprise Hub</span>
          </div>
        </div>
      </div>
            ...
            <div key={group.key} style={{ marginBottom: "0.8rem" }}>
              <div 
                ...
                style={{ 
                  ...
                  color: isExpanded ? T.text : T.muted, padding: "0.5rem 1.5rem",
              ...
              <div style={{ 
                ...
                {group.items.map(({ path, label, icon: Icon }) => (
                  <NavItem key={path} href={path} label={label} icon={Icon} active={isActive(path)} />
                ))}
              </div>
            </div>
          );
      ...
      {/* User Profile / Logout */}
      <div style={{ borderTop: `1px solid ${T.border}`, background: "rgba(0,255,136,0.02)" }}>
        <div style={{ padding: "1.5rem" }}>
          ...
          <Link href="/" style={{ textDecoration: 'none', display: "block", marginTop: "1rem" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "0.8rem", cursor: "pointer", color: "#ff3355", transition: "opacity 0.2s" }}>
              <LogOut size={16} />
              <span style={{ fontFamily: T.mono, fontSize: "0.8rem", letterSpacing: "0.05em", fontWeight: 600 }}>Log out</span>
            </div>
          </Link>
        </div>
      </div>
...
function NavItem({ href, label, icon: Icon, active }: { href: string, label: string, icon: React.ElementType, active: boolean }) {
  ...
  return (
    <Link href={href} 
      ...
      style={{
        display: "flex", alignItems: "center", gap: "0.8rem", padding: "0.6rem 1.5rem", textDecoration: "none",
        fontFamily: T.mono, fontSize: "0.78rem", letterSpacing: "0.05em",
        background: active ? "rgba(0,255,136,0.1)" : (hov ? "rgba(255,255,255,0.03)" : "transparent"),
    ...
    >
      ...
      <span style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
        {active && <span style={{ display: "inline-block", width: 6, height: 6, borderRadius: "50%", background: T.g }} />}
        {label}
      </span>
    </Link>
  );
