...
export const outreachWorker = new Worker(
  ...
  async (job: Job) => {
    ...
    console.log(
      `[OUTREACH] Generating outreach for lead ${lead_id} on campaign ${campaign_id}`,
    );
  ...
  `;
    ...
    try {
      ...
      console.warn(
        `[OUTREACH] LLM generation failed, falling back to mock text. Error: ${err}`,
      );
    ...
    console.log(
      `[OUTREACH] Outreach generated and sent for lead ${lead.id}. Event ID: ${eventId}`,
    );
