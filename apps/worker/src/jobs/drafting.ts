...
import { leads, drafts, campaigns, suppression_list, companies, organizations } from '@ai-workforce/db/schema';
import { eq, and, or } from 'drizzle-orm';
import { generateText } from '@ai-workforce/llm';

const REDIS_URL = process.env.REDIS_URL || 'redis://127.0.0.1:6379';

const SUPERLATIVES = ['best', 'fastest', 'cheapest', 'greatest', 'most', 'revolutionary', 'groundbreaking', 'game-changing', 'game changing'];
const BANNED_OPENERS = ['hope this email finds you well', 'finds you well', 'hope you are doing well'];
...
function runValidations(subject: string, body: string, researchBrief: any) {
  ...
  results.push({
    rule: 'No clickbait',
    passed: !lowerSubject.includes('urgent') && !lowerSubject.includes('quick question') && !lowerSubject.includes('important'),
...
Proof Points: ${icpData?.proofPoints || 'We save 20% on infra costs.'}`;
      ...
      if (!match) {
        ...
        return { subject: `Connecting with ${lead.company}`, body: res.content + '\n[unsubscribe]' };
      }
    ...
    // We want 2 variants
    const hooks = (lead.research_brief as any).hooks || ['Noticed your recent growth.'];
    ...
    for (const hook of selectedHooks) {
      ...
      while (attempt < 2 && !passedAll) {
        attempt++;
        const failReasonsStr = valResults.filter(v => !v.passed).map(v => `- ${v.rule}: ${v.message}`).join('\n');
        variant = await generateVariant(hook, attempt > 1 ? failReasonsStr : undefined);
        
        valResults = runValidations(variant.subject, variant.body, lead.research_brief);
    ...
    const bestVariantData = variantsData.find(v => v.passed_all) || variantsData[0];
    ...
    console.log(`[Drafting] Complete for lead ${leadId}. Status: ${finalStatus}`);
