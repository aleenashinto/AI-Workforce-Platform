...
export default async function dataRetentionProcessor(job: Job) {
  ...
  const orgs = await db
    .select({
      id: organizations.id,
      retention_days: organizations.retention_days,
    })
    .from(organizations)
    .where(sql`retention_days IS NOT NULL`);
  ...
  for (const org of orgs) {
    ...
    const cutoffDate = subDays(
      new Date(),
      parseInt(org.retention_days.toString(), 10),
    );

    job.log(
      `Processing org ${org.id}, cutoff date: ${cutoffDate.toISOString()}`,
    );
    ...
    await withTenant(db, org.id, async (tx) => {
      ...
      await tx
        ...
        .where(
          sql`${conversations.org_id} = ${org.id} AND ${conversations.created_at} < ${cutoffDate.toISOString()}`,
        );
      ...
      await tx
        .delete(audit_logs)
        .where(
          sql`${audit_logs.org_id} = ${org.id} AND ${audit_logs.created_at} < ${cutoffDate.toISOString()}`,
        );
    });
  }
