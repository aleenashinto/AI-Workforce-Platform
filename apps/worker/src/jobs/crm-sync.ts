...
export const crmSyncWorker = new Worker(
  ...
  async (job: Job) => {
    ...
    console.log(
      `[CRMSync] Starting 2-way sync for integration ${integrationId}`,
    );
    ...
    const crmContacts = await provider.pullContacts(
      integrationId,
    ...
    for (const lead of syncableLeads) {
      if (lead.status !== "new") {
        const success = await provider.pushLead(
          integrationId,
          lead,
          integration.credentials,
        );
    ...
    await db
      ...
      .where(eq(integrations.id, integrationId));
    console.log(
      `[CRMSync] Finished. Suppressed ${suppressedCount} new domains, Pushed ${pushedCount} leads.`,
    );
