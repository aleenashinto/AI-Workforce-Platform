...
export const classifyReplyWorker = new Worker(
  ...
  async (job: Job) => {
    ...
    console.log(
      `[ClassifyReply] Classifying incoming reply for lead ${leadId}`,
    );
...
Respond ONLY with the category keyword.`;
    ...
    const finalClass = validClasses.includes(classification)
      ? classification
      : "auto_reply";
    ...
    if (finalClass === "bounce") {
      ...
      if (mailbox) {
        const metrics = (mailbox.metrics as any) || {
          bounces: 0,
          sent_total: 100,
        };
        metrics.bounces += 1;
        ...
        if (bounceRate > 0.03) {
          await db
            .update(mailboxes)
            .set({ status: "paused", metrics })
            .where(eq(mailboxes.id, mailboxId));
          console.warn(
            `[ClassifyReply] Mailbox ${mailboxId} auto-paused due to high bounce rate (${(bounceRate * 100).toFixed(1)}%).`,
          );
        } else {
          await db
            .update(mailboxes)
    ...
    if (finalClass === "not_interested" || finalClass === "unsubscribe") {
      ...
      if (domain) {
        ...
        console.log(
          `[ClassifyReply] Suppressed domain ${domain} due to ${finalClass}`,
        );
      }
    }
