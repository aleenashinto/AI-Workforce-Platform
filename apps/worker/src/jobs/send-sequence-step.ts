...
import { leads, drafts, suppression_list, outreach_events, mailboxes, sequences } from '@ai-workforce/db/schema';
...
function hasLegalFooter(body: string) {
  ...
  return (lower.includes('unsubscribe') || lower.includes('opt out')) && (lower.match(/\d+.*street|ave|road|blvd/)); 
...
export const sendSequenceWorker = new Worker(
  ...
  async (job: Job) => {
    ...
    console.log(`[SendSequence] Processing draft ${draftId} from mailbox ${mailboxId}`);
    ...
    const effectiveCap = WarmupCalculator.getEffectiveCap(mailbox.created_at || new Date(), Number(mailbox.daily_cap));
    
    if (dailySent >= effectiveCap) {
      console.warn(`[SendSequence] Mailbox ${mailboxId} reached effective cap of ${effectiveCap}. Deferring.`);
      return { success: false, reason: 'cap_reached' };
    }

    // 3. Legal Footer Validation
    if (!hasLegalFooter(draft.body || '')) {
      await db.update(drafts).set({ status: 'failed_validation' }).where(eq(drafts.id, draftId));
    ...
    await db.transaction(async (tx) => {
      ...
      const checks = [];
      if (domain) checks.push(eq(suppression_list.entity_value, domain.toLowerCase()));
      if (lead.email) checks.push(eq(suppression_list.entity_value, lead.email.toLowerCase()));
      ...
      if (found) {
        suppressed = true;
        await tx.update(leads).set({ status: 'suppressed' }).where(eq(leads.id, lead.id));
        await tx.update(drafts).set({ status: 'rejected' }).where(eq(drafts.id, draftId));
      } else {
        ...
        await tx.update(drafts).set({ status: 'sent' }).where(eq(drafts.id, draftId));
        await tx.update(leads).set({ status: 'contacted' }).where(eq(leads.id, lead.id));
        
        // Update metrics
        const newMetrics = { ...((mailbox.metrics as any) || {}), sent_today: dailySent + 1 };
        await tx.update(mailboxes).set({ metrics: newMetrics }).where(eq(mailboxes.id, mailboxId));
      }
    });

    if (suppressed) {
      console.warn(`[SendSequence] Unbypassable check caught suppressed lead ${lead.id}. Aborted.`);
