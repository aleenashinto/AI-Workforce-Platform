...
import {
  ApolloLeadProvider,
  DiscoveredLead,
} from "@ai-workforce/core/providers/lead-provider";
...
export const discoverLeadsWorker = new Worker(
  ...
  async (job: Job) => {
    ...
    const validDomains = fetchedCompanies.filter(
      (c) => !FREE_MAIL_DOMAINS.includes(c.domain.toLowerCase()),
    );
    ...
    const existingCompanies = await db.query.companies.findMany({
      where: and(
        ...
        inArray(companies.domain, domainNames),
      ),
    });
    ...
    const suppressed = await db.query.suppression_list.findMany({
      where: and(
        ...
        inArray(suppression_list.entity_value, domainNames),
      ),
    });

    const existingDomains = new Set(
      existingCompanies.map((c) => c.domain.toLowerCase()),
    );
    const suppressedDomains = new Set(
      suppressed.map((s) => s.entity_value.toLowerCase()),
    );

    const newCompanies = validDomains.filter(
      (c) =>
        !existingDomains.has(c.domain.toLowerCase()) &&
        !suppressedDomains.has(c.domain.toLowerCase()),
    );

    const companyIdMap = new Map<string, string>();
    existingCompanies.forEach((c) =>
      companyIdMap.set(c.domain.toLowerCase(), c.id),
    );
    ...
    const suppressedEmails = await db.query.suppression_list.findMany({
      where: and(
        eq(suppression_list.org_id, orgId),
        eq(suppression_list.entity_type, "email"),
        inArray(suppression_list.entity_value, emails),
      ),
    });
    const suppressedEmailSet = new Set(
      suppressedEmails.map((s) => s.entity_value.toLowerCase()),
    );
    ...
    for (const contact of validContacts) {
      ...
      const existingLead = await db.query.leads.findFirst({
        where: and(
          ...
          eq(leads.email, contact.email.toLowerCase()),
        ),
      });
