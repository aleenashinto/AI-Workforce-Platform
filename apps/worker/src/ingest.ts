...
export async function processIngestJob(job: Job, embedQueue: Queue) {
  ...
  // 4. Upsert document
  const existingDocs = await db.select().from(knowledge_documents).where(eq(knowledge_documents.source_id, source_id));
  let documentId;
  ...
  if (existingDocs.length > 0 && existingDocs[0].content_hash === contentHash) {
    ...
    await db.update(knowledge_documents).set({ content_hash: contentHash, sync_status: 'processing' }).where(eq(knowledge_documents.id, documentId));
  } else {
    const [newDoc] = await db.insert(knowledge_documents).values({
      source_id,
      org_id,
  ...
  await db.update(knowledge_sources).set({ status: 'processing', updated_at: new Date() }).where(eq(knowledge_sources.id, source_id));
