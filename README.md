...
LLM Gateway (packages/llm): A centralized package to handle communication with Large Language Models (like OpenAI's GPT models or Anthropic) for generating responses and evaluating data. 3. Functionality by Module & Page
...
Mailboxes (/sales-assistant/mailboxes): Connect external email accounts (via SMTP/IMAP or OAuth) so the AI can send emails directly from the user's real email address. 4. Key Workflows
