...
export class ExaSearchProvider implements SearchProvider {
  ...
  async searchCompanyNews(
    ...
    if (!this.apiKey) {
      console.warn(
        "[ExaSearchProvider] EXA_API_KEY missing. Returning mock news.",
      );
      return [
        {
  ...
  async searchCompanyProduct(
    ...
    if (!this.apiKey) {
      console.warn(
        "[ExaSearchProvider] EXA_API_KEY missing. Returning mock product data.",
      );
      return [
        {
    ...
    try {
      ...
      console.error(
        "[ExaSearchProvider] Failed to search company products:",
        err,
      );
      return [];
    }
