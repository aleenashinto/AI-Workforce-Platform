...
describe('authz', () => {
  describe('authorize()', () => {
    ...
    it('returns true when user has one valid role among many', () => {
      expect(authorize(['viewer', 'support_agent'], 'RESPOND_TO_CONVERSATIONS')).toBe(true);
    });

    it('returns false when user has no valid roles', () => {
      expect(authorize(['viewer', 'support_agent'], 'APPROVE_DRAFT')).toBe(false);
    });
  ...
  describe('enforce()', () => {
    it('throws if unauthorized', () => {
      expect(() => enforce(['viewer'], 'MANAGE_BILLING')).toThrowError('Unauthorized');
    });
