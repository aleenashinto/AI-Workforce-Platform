...
const companyNames = [
  "NovaStack Technologies", "CloudBridge AI", "FinCore Systems", "BrightCommerce", 
  "DataSphere Labs", "GreenGrid Solutions", "HealthSync Technologies", "MarketPilot", 
  "Vertex Analytics", "QuantumFlow", "BlueOrbit Software", "Nexora Systems", 
  "PixelForge", "SecureLayer", "Optima Cloud", "ScaleWorks", "CoreVista", 
  "RapidDesk", "IntelliRoute", "FusionPeak", "NorthStar Digital", "CloudNest Labs", 
  "BrightPath AI", "DataForge Systems", "FlowMetric", "TechVista", "CodeHarbor", 
  "OrbitStack", "InsightWorks", "ElevateCloud"
];

const industries = ["SaaS", "FinTech", "Healthcare", "E-commerce", "AI", "Cybersecurity", "Education", "Manufacturing"];
const locations = ["San Francisco", "New York", "London", "Berlin", "Austin", "Singapore", "Toronto"];

const firstNames = ["Alex", "Maria", "Sarah", "James", "Emily", "Daniel", "Michael", "Emma", "John", "Jessica", "David", "Sophia"];
const lastNames = ["Johnson", "Wilson", "Thomas", "Anderson", "Carter", "Miller", "Brown", "Davis", "Smith", "Taylor", "White", "Harris"];
const jobTitles = ["CEO", "Founder", "CTO", "CIO", "CMO", "VP Engineering", "VP Sales", "Head of Marketing", "Head of Operations", "Procurement Manager", "IT Director"];

const getRandomInt = (min, max) => Math.floor(Math.random() * (max - min + 1)) + min;
const getRandomItem = (arr) => arr[getRandomInt(0, arr.length - 1)];

async function generateDemoData() {
  console.log("🌱 Starting Demo Data Seed for AI Sales Assistant...");

  await sqlClient\SELECT set_config('app.current_org_id', '\', false)\;

  console.log("1. Checking for existing demo data...");
  const existingCompanies = await db.select({ id: schema.companies.id }).from(schema.companies).where(sql\metadata->>'demo' = '\'\);
  ...
  const campaignNames = ["European SaaS Outreach", "FinTech CTO Campaign", "AI Startup Campaign", "Enterprise Support Campaign", "AWS Technology Campaign"];
  ...
  for (let i = 0; i < 75; i++) {
    ...
    let qualification = "";
    if (score > 80) qualification = "Strong ICP match. The company operates in the target segment and has recently expanded.";
    ...
    const [l] = await db.insert(schema.leads).values({
      ...
      metadata: { demo: DEMO_TAG, source: "AI Discovery", contact_id: cInfo.id, qualification }
    }).returning({ id: schema.leads.id });
    createdLeadIds.push({ id: l.id, companyId: cInfo.companyId, contactId: cInfo.id });
  }

  // CREATE BUYING SIGNALS
  console.log("7. Creating Buying Signals...");
  const signalTypes = ["Funding", "Hiring", "Expansion", "Technology Migration", "Leadership Change"];
  for (let i = 0; i < 50; i++) {
  ...
  const oppStages = ["Qualified", "Meeting", "Proposal", "Negotiation", "Won", "Lost"];
  const createdOppIds = [];
  for (let i = 0; i < 25; i++) {
    const lInfo = getRandomItem(createdLeadIds);
    const stage = getRandomItem(oppStages);
    const value = getRandomItem([180000, 320000, 480000, 750000, 1200000]);
    const [opp] = await db.insert(schema.opportunities).values({
      ...
      currency: "INR",
      stage: stage,
      probability: stage === "Won" ? "100" : (stage === "Lost" ? "0" : getRandomInt(20, 80).toString())
  ...
  for (let i = 0; i < 25; i++) {
    await db.insert(schema.follow_ups).values({
      ...
      company_id: getRandomItem(createdCompanyIds),
      owner_id: getRandomItem(createdUsers),
      status: getRandomItem(["pending", "completed"]),
...
async function resetDemoData() {
  console.log("🗑️ Starting Reset of Sales Demo Data...");
  
  await sqlClient\SELECT set_config('app.current_org_id', '\', false)\;
  
  console.log("Finding demo companies...");
  const companies = await db.select({ id: schema.companies.id }).from(schema.companies).where(sql\metadata->>'demo' = '\'\);
  ...
  await db.delete(schema.ai_recommended_actions).where(sql\	rue\);
  await db.delete(schema.ai_sales_insights).where(sql\	rue\);
  
  for (const cId of cIds) {
    await db.delete(schema.outreach_events).where(sql\lead_id IN (SELECT id FROM leads WHERE company_id = '\')\);
    await db.delete(schema.meetings).where(sql\company_id = '\'\);
    await db.delete(schema.follow_ups).where(sql\company_id = '\'\);
    await db.delete(schema.sales_activities).where(sql\company_id = '\'\);
    await db.delete(schema.opportunities).where(sql\company_id = '\'\);
    await db.delete(schema.buying_signals).where(sql\company_id = '\'\);
    await db.delete(schema.leads).where(sql\company_id = '\'\);
    await db.delete(schema.contacts).where(sql\company_id = '\'\);
    await db.delete(schema.companies).where(sql\id = '\'\);
  }
...
fs.writeFileSync('packages/db/seed-sales.ts', seedScript);
fs.writeFileSync('packages/db/reset-sales.ts', resetScript);
