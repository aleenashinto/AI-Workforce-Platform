  ...
  try {
    ...
    for (const s of seqs) {
      ...
      await sqlClient`
        ...
        VALUES (${uuidv4()}, ${s.id}, 0, 'Initial Outreach', 'email', 'email', 'Subject: Hi\nBody: Hello')
      `;
      await sqlClient`
        INSERT INTO sequence_steps (id, sequence_id, day_offset, name, type, channel, template)
        VALUES (${uuidv4()}, ${s.id}, 2, 'Wait 2 Days', 'wait', 'wait', '')
      `;
      await sqlClient`
        INSERT INTO sequence_steps (id, sequence_id, day_offset, name, type, channel, template)
        VALUES (${uuidv4()}, ${s.id}, 2, 'AI Follow-up', 'ai_email', 'email', 'Subject: Follow up\nBody: AI Generated')
      `;
      ...
      if (s.status !== "draft") {
        ...
        for (let i = 0; i < leads.length; i++) {
          ...
          await sqlClient`
            ...
            VALUES (${uuidv4()}, ${org_id}, ${s.id}, ${leads[i].id}, ${status}, ${currentStep})
          `;
